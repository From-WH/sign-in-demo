...
var server = http.createServer(function(request, response){
  ...
  if(path === '/'){
    let string = fs.readFileSync('./index.html', 'utf8') //读取一下当前目录下的index.html文件 以utf8的编码读取
    let cookies =  request.headers.cookie.split('; ') // 获得所有用户的cookie 如：['email=1@', 'a=1', 'b=2']
    let hash = {}
    ...
    readBody(request).then((body)=>{
      ...
      let found
      for(let i=0;i<users.length; i++){
        if(users[i].email === email && users[i].password === password){  //如果email和密码都正确
          found = true
          break
        }
      }
